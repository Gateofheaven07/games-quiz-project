...
router.get('/questions', async (req, res) => {
    try {
        ...
        const numLimit = Math.min(parseInt(limit) || 10, 20);
        const result = await query(`SELECT id, text, options, correct_answer, category, difficulty 
       FROM questions 
       WHERE difficulty = $1 
       ORDER BY RANDOM() 
       LIMIT $2`, [difficulty, numLimit]);
...
router.post('/results', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await query(`INSERT INTO game_results 
       (user_id, opponent_id, score, correct_answers, total_questions, duration, difficulty)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING id, user_id, opponent_id, score, correct_answers, total_questions, duration, difficulty, created_at`, [req.user.userId, opponentId || null, score, correctAnswers, totalQuestions, duration, difficulty || 'medium']);
        const gameResult = {
            id: result.rows[0].id,
...
router.get('/history', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await query(`SELECT id, user_id, opponent_id, score, correct_answers, total_questions, duration, difficulty, created_at
       FROM game_results 
       WHERE user_id = $1 
       ORDER BY created_at DESC
       LIMIT $2 OFFSET $3`, [req.user.userId, numLimit, numOffset]);
        const countResult = await query('SELECT COUNT(*) as count FROM game_results WHERE user_id = $1', [req.user.userId]);
        const gameResults = result.rows.map((row) => ({
            id: row.id,
            userId: row.user_id,
